...
function getUrgencyColor(daysUntil: number, isUrgent: boolean) {
  if (isUrgent || daysUntil < 0) return { dot: "bg-[#DC2626]", text: "text-[#DC2626]", label: daysUntil < 0 ? "Overdue" : "Urgent" };
...
export function Sidebar({ activePage, onNavigate, onViewJobbing, jobbings, onNewJobbing, onLogout, role, darkMode, onToggleDarkMode, shopName }: SidebarProps) {
    ...
    <aside className="w-[260px] shrink-0 flex flex-col h-full bg-white dark:bg-[#1a1a1a] border-r border-[rgba(0,0,0,0.06)] dark:border-[rgba(255,255,255,0.06)]">
      ...
      <div className="px-5 pt-6 pb-5 border-b border-[rgba(0,0,0,0.06)] dark:border-[rgba(255,255,255,0.06)]">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl bg-[#778873] flex items-center justify-center shrink-0 overflow-hidden shadow-sm">
            <img src="/logo.jpg" alt="AJ Graphica" className="w-full h-full object-cover" />
          </div>
          <div>
            <div className="text-base font-bold leading-tight text-[#778873] tracking-widest uppercase">{shopName}</div>
            <div className="text-xs text-gray-400 leading-tight">Print & Tailoring</div>
          </div>
        </div>
        {role === "superadmin" && (
          <div className="mt-2 text-[10px] font-semibold text-[#778873] dark:text-[#A1BC98] tracking-wider uppercase">Superadmin</div>
        )}
      </div>
      ...
      <div className="px-3 pt-4 pb-2">
        <button
          ...
          className="w-full bg-gradient-to-r from-[#778873] to-[#A1BC98] hover:from-[#DC2626] hover:to-[#778873] text-white text-base py-3 px-4 rounded-lg flex items-center justify-center gap-2 transition-all duration-200 font-medium shadow-sm hover:shadow-md hover:scale-[1.02]"
        >
          <Plus size={18} />
          New Jobbing
        </button>
        ...
        {navItems.map((item) => (
          <button
            ...
            className={`relative flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm w-full text-left transition-all duration-200 ${
              activePage === item.id
                ? "bg-[#FDF6ED] dark:bg-[#3a1010] text-[#778873] dark:text-[#A1BC98] font-semibold"
                : "text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-[#2a2a2a] hover:translate-x-0.5"
            }`}
          >
            {activePage === item.id && (
              <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 bg-[#778873] dark:bg-[#A1BC98] rounded-r-full" />
            )}
            <span className={
              activePage === item.id
            ...
            {upcoming.map((j) => {
              ...
              return (
                <div
                  ...
                  className={`px-2 py-2 rounded text-xs flex items-start gap-2 transition-colors cursor-pointer ${
                    j.isUrgent || j.daysUntil < 0
                      ? "bg-[#FDF6ED] dark:bg-[#3a1010]"
                      : j.daysUntil === 0
      ...
      <div className="px-4 py-3 border-t border-[rgba(0,0,0,0.06)] dark:border-[rgba(255,255,255,0.06)] flex items-center gap-2">
        <div className="w-8 h-8 rounded-full bg-[#778873] flex items-center justify-center text-white text-xs font-bold shrink-0">
        ...
        <button
          onClick={onLogout}
          title="Sign out"
          className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 dark:hover:bg-[#2a2a2a] text-gray-400 hover:text-[#778873] dark:hover:text-[#A1BC98] transition-all duration-200"
        >
          <LogOut size={16} />
        </button>
      </div>
