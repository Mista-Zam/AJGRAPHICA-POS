...
interface EditJobbingModalProps {
  jobbing: Jobbing;
  onClose: () => void;
...
export function EditJobbingModal({ jobbing, onClose, onUpdate }: EditJobbingModalProps) {
  const [form, setForm] = useState({
    customerName: jobbing.customerName,
    contactNumber: jobbing.contactNumber,
    address: jobbing.address ?? "",
    jobType: jobbing.jobType,
    fabric: jobbing.fabric,
    ...
    isUrgent: jobbing.isUrgent,
    attachment: jobbing.attachment,
  });
          ...
          <div className="px-4 sm:px-5 py-3 sm:py-4 flex flex-col gap-3 sm:gap-4 flex-1">
            ...
            <div>
              ...
              <input required value={form.customerName} onChange={(e) => set("customerName", e.target.value)}
                placeholder="e.g. Barangay Hall Sta. Cruz"
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-3">
              <div>
                <label className="block text-[10px] sm:text-xs font-medium text-gray-600 dark:text-gray-400 mb-0.5 sm:mb-1">Amount (₱) *</label>
            ...
            {form.amount > 0 && (
              <div className={`text-xs sm:text-sm rounded-xl px-2.5 sm:px-3 py-2 sm:py-2.5 flex items-center justify-between ${balance > 0 ? "bg-[#FEE2E2] dark:bg-[#3a1010] text-[#DC2626] dark:text-[#e87070]" : "bg-[#E8F5F1] dark:bg-[#064E3B] text-[#0F6E56] dark:text-[#5abb9e]"}`}>
                <span className="text-[10px] sm:text-xs font-medium">{balance > 0 ? "Remaining Balance" : "Fully Paid"}</span>
                <span className="font-semibold text-xs sm:text-sm">₱{Math.max(0, balance).toLocaleString("en-PH")}</span>
              </div>
            )}
          </div>
